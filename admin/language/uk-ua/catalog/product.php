<?php
// Заголовок
$_['heading_title'] = 'Товари';

// Текст
$_['text_success'] = 'Успіх: Ви змінили товари!';
$_['text_variant'] = 'Зверніть увагу, оскільки це варіант товару, ви повинні перевизначити дані <a href="%s" target="_blank" class="alert-link">основного товару</a>. Інакше дані будуть замінені на дані <a href="%s" target="_blank" class="alert-link">основного товару</a>.';
$_['text_list'] = 'Список товарів';
$_['text_add'] = 'Додати товар';
$_['text_edit'] = 'Редагувати товар';
$_['text_filter'] = 'Фільтр';
$_['text_default'] = 'За замовчуванням';
$_['text_option'] = 'Опція';
$_['text_option_value'] = 'Значення опції';
$_['text_percent'] = 'Відсоток';
$_['text_amount'] = 'Фіксована сума';
$_['text_keyword'] = 'Не використовуйте пробіли, замініть їх на тире, і переконайтеся, що SEO URL є унікальним.';
$_['text_recurring'] = 'Періодичні профілі';

// Стовпці
$_['column_name'] = 'Назва товару';
$_['column_model'] = 'Зовнішній ID';
$_['column_image'] = 'Зображення';
$_['column_price'] = 'Ціна';
$_['column_quantity'] = 'Кількість';
$_['column_status'] = 'Статус';
$_['column_action'] = 'Дія';

// Поля вводу
$_['entry_name'] = 'Назва товару';
$_['entry_description'] = 'Опис';
$_['entry_meta_title'] = 'Мета-тег Title';
$_['entry_meta_keyword'] = 'Мета-тег Keywords';
$_['entry_meta_description'] = 'Мета-тег Description';
$_['entry_store'] = 'Магазини';
$_['entry_keyword'] = 'SEO URL';
$_['entry_model'] = 'Зовнішній ID';
$_['entry_sku'] = 'SKU';
$_['entry_upc'] = 'UPC';
$_['entry_ean'] = 'EAN';
$_['entry_jan'] = 'JAN';
$_['entry_isbn'] = 'ISBN';
$_['entry_mpn'] = 'MPN';
$_['entry_location'] = 'Розташування';
$_['entry_shipping'] = 'Потребує доставку';
$_['entry_manufacturer'] = 'Виробник';
$_['entry_date_available'] = 'Дата доступності';
$_['entry_quantity'] = 'Кількість';
$_['entry_minimum'] = 'Мінімальна кількість';
$_['entry_stock_status'] = 'Статус відсутності на складі';
$_['entry_price'] = 'Ціна';
$_['entry_tax_class'] = 'Податкова категорія';
$_['entry_points'] = 'Бали';
$_['entry_option_points'] = 'Бали';
$_['entry_subtract'] = 'Віднімати зі складу';
$_['entry_weight_class'] = 'Одиниця ваги';
$_['entry_weight'] = 'Вага';
$_['entry_dimension'] = 'Розміри (Д x Ш x В)';
$_['entry_length_class'] = 'Одиниця довжини';
$_['entry_length'] = 'Довжина';
$_['entry_width'] = 'Ширина';
$_['entry_height'] = 'Висота';
$_['entry_image'] = 'Зображення';
$_['entry_additional_image'] = 'Додаткові зображення';
$_['entry_customer_group'] = 'Група клієнтів';
$_['entry_date_start'] = 'Дата початку';
$_['entry_date_end'] = 'Дата завершення';
$_['entry_priority'] = 'Пріоритет';
$_['entry_attribute'] = 'Атрибут';
$_['entry_attribute_group'] = 'Група атрибутів';
$_['entry_text'] = 'Текст';
$_['entry_option'] = 'Опція';
$_['entry_option_value'] = 'Значення опції';
$_['entry_required'] = 'Обов’язково';
$_['entry_status'] = 'Статус';
$_['entry_sort_order'] = 'Порядок сортування';
$_['entry_category'] = 'Категорії';
$_['entry_filter'] = 'Фільтри';
$_['entry_download'] = 'Завантаження';
$_['entry_related'] = 'Схожі товари';
$_['entry_tag'] = 'Теги товару';
$_['entry_reward'] = 'Бали винагороди';
$_['entry_layout'] = 'Макет';
$_['entry_recurring'] = 'Періодичний профіль';

// Допомога
$_['help_sku'] = 'Складський обліковий номер';
$_['help_upc'] = 'Універсальний код товару';
$_['help_ean'] = 'Європейський артикул';
$_['help_jan'] = 'Японський артикул';
$_['help_isbn'] = 'Міжнародний стандартний номер книги';
$_['help_mpn'] = 'Номер виробника';
$_['help_manufacturer'] = '(Автодоповнення)';
$_['help_minimum'] = 'Примусити мінімальну кількість замовлення';
$_['help_stock_status'] = 'Статус, показаний, коли товар відсутній на складі';
$_['help_points'] = 'Кількість балів, необхідних для придбання цього товару. Якщо не бажаєте, щоб цей товар можна було придбати за бали, залиште 0.';
$_['help_category'] = '(Автодоповнення)';
$_['help_filter'] = '(Автодоповнення)';
$_['help_download'] = '(Автодоповнення)';
$_['help_related'] = '(Автодоповнення)';
$_['help_tag'] = 'Розділяйте теги комами';

// Помилки
$_['error_warning'] = 'Увага: Будь ласка, уважно перевірте форму на наявність помилок!';
$_['error_permission'] = 'Увага: У вас немає дозволу на зміну товарів!';
$_['error_name'] = 'Назва товару повинна містити від 1 до 255 символів!';
$_['error_meta_title'] = 'Мета-тег Title повинен містити від 1 до 255 символів!';
$_['error_model'] = 'Зовнішній ID має містити від 1 до 64 символів!';
$_['error_model_unique'] = 'Товар з таким "Зовнішнім ID" вже існує!';
$_['error_keyword'] = 'SEO URL вже використовується!';
$_['error_unique'] = 'SEO URL повинен бути унікальним!';
